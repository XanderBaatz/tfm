...
from kfm.data.utils import save_images, save_json


def get_wandb_logger(trainer: Trainer) -> WandbLogger | None:
    wandb_logger = None
        ...
        if len(outputs):
            ...
            for a in atoms:
                try:
                    ...
                    warnings.warn(f"In {self.__class__} the following error occurred: {e}")
...
class LogMetricsDetails(Callback):
    def __init__(self, dirpath: Path | str, fname: Path | str):
        self.dirpath = dirpath
        self.fname = fname
