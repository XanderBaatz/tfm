...
class SampleDatasetDNG(torchdata.Dataset):
    ...
    def from_npy(
        ...
        """Load empirical distribution from a .npy file saved by preprocess_csv."""
        empirical_distribution = np.load(dist_path)
        return cls(
...
class SampleDatasetCSP(torchdata.Dataset):
    def __init__(
        ...
        self.transform = transform
        self.parsed_formulas = [chemparse.parse_formula(f) for f in formulas for _ in range(n_samples_per_formula)]

    def __getitem__(self, idx):
        formula: dict[str, float] = self.parsed_formulas[idx]
        h = torch.LongTensor([atomic_numbers[el] for el in formula for _ in range(int(formula[el]))])
