...
class ContinuousIntervalLengths(BaseTransform):
    ...
    def __init__(
        ...
        if normalize_by_num_atoms:
            assert lengths_loc_scale is None, "'normalize_by_num_atoms' and 'lengths_loc_scale' cannot be combined"
    ...
    def forward(self, data: Data) -> Data:
        ...
        if self.lengths_loc_scale:
            n = len(data.pos)
            loc, scale = self.lengths_loc_scale[n]  # loc, scale are 3-dimensional for a, b, c
...
@functional_transform("to_kinetic_crystal_state")
class ToKineticCrystalState(BaseTransform):
