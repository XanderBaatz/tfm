...
def validity_structure(structure: Structure, cutoff: float = 0.5):
    ...
    if distance_matrix.min() < cutoff or structure.volume < 0.1 or max(structure.lattice.abc) > 40:
        return False
    return True


def _validity_composition(unique_symbols, count, use_pauling_test=True, include_alloys=True):
    """Taken from DiffCSP."""
...
def wdist_num_elements(input_s: list[Structure], target_s: list[Structure]):
    ...
    pred_num_elements = [len(set(s.species)) for s in input_s]
    gt_num_elements = [len(set(s.species)) for s in target_s]
